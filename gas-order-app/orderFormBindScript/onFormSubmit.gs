...
function onFormSubmit(e) {
  ...
  // 注文データ追加前の最終行を取得しておく
  const num_last_row = order_sheet.getLastRow();
  debug(`4. 注文データ追記前の最終行番号: ${num_last_row}`);

  // 取得したタイムスタンプから date(年月日) を抽出
  const accepted_date = Utilities.formatDate(timestamp, "JST", "yyyyMMdd");  // 年月日
  // order_sheet.getRange(num_last_row + 1, order_sheet_config.ACCEPTED_DATE_COLUMN).setValue(accepted_date);

  // // フォームの回答結果をシートに転記
  // debug("転記開始");
  // received_order.forEach((order) => {
  //   // 質問タイトルに対応する、シートの列番号を取得
  //   const column_idx = getColumnNumberByTitle(order.question, new_headers);
  //   if (column_idx !== -1) {
  //     order_sheet.getRange(num_last_row + 1, column_idx).setValue(order.answer);
  //   }
  // });
  // debug("5. 転記完了");
  ...
  new_menu_headers.forEach(header => {  // ヘッダーのメニュー要素を一つ一つ取得して、
    ...
    order_values.push(form_res ? form_res.answer : "");  // form_resがtrue(回答がある)ならその回答を、falseなら空白文字列をpush
  })
  
  // 注文データを　order_shee 追加
  debug(order_values);
